...
Require Node 20 or later. Do not use the unscoped npm package `vibekit`; it is unrelated. Version `0.1.0` of `@useagentsio/cli` has a broken global bin, so use `0.1.1` or later.
...
Read `.vibekit/project.yaml`, `.vibekit/installed.json`, and the relevant installed Agent or Component files. Keep the four list states separate: installed, configured, available, and verified.
...
Accept selectors as `type name`, `type:name`, or `type:name@version`. Accept `-y` as the short form of `--yes`. Require `--yes` for noninteractive `add`, `update`, and `remove` operations.
