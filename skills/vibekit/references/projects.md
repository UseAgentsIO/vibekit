...
## Select an official provider and Component

- Use `provider:openai-codex` for Pi's ChatGPT/Codex OAuth after authenticating through Pi `/login`.
- Use `provider:xai` for Pi OAuth through `/login`, or provide the optional `XAI_API_KEY` environment reference.
- Use `provider:openai`, `provider:opencode-go`, or `provider:openrouter` with `OPENAI_API_KEY`, `OPENCODE_API_KEY`, or `OPENROUTER_API_KEY` respectively.

Install the provider, set a usable Project default model, and verify the actual Pi route. Installing provider metadata alone does not prove authentication or model availability.

Use `tool:filesystem` for source reads/writes, `tool:execution` for bounded commands, and `tool:github` for repository operations. Use `skill:software-development` or `skill:research` for procedures, `interface:terminal` for V1 interaction, `state:repository` for durable State, `policy:least-privilege` and `policy:require-verification` for constraints, and `verifier:command` for deterministic checks. Install only what the selected Agents and Project actually require.
...
Resolve a model in this order: allowed Task override, Project Agent binding, Agent default, Project default, then error. Honor `allowTaskOverride` and `allowProjectOverride`; do not infer that a catalog entry is runnable without live authentication and model verification.
