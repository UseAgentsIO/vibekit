...
- `packages/interface-terminal` — terminal Interface
...
Do not fork Pi. Do not add a marketplace, third-party registries, graphical builder, `orchestrator` type, `subagent` type, or `Blocks`. Do not implement Slack or Telegram unless explicitly asked.
...
Phases 1–8 are sequential. Implement only the current phase's exit criteria. Slack and Telegram are out of scope unless explicitly requested.
