...
export function authorizeInvocation(input: {
  ...
  if (grant === undefined) {
    throw new VibeKitError({
      ...
      message:
        `Capability ${input.invocation.capability} is not in the effective grant set: ` +
        `${capabilityLabel(input.invocation.capability)} is not enabled. ` +
        `Enable ${capabilitySetting(input.invocation.capability)} or choose a preset that includes it.`,
      details: {
  ...
  if (input.invocation.moduleId !== undefined && grant.provider !== undefined) {
    if (input.invocation.moduleId !== grant.provider) {
      throw new VibeKitError({
        ...
        message:
          `The requested tool is not the configured provider for ${capabilityLabel(input.invocation.capability)}. ` +
          `Use the Project's configured ability provider or update that binding in advanced settings.`,
        details: {
    ...
    if (grant.scope.branches !== undefined && grant.scope.branches.length > 0) {
      ...
      if (branch !== undefined && !grant.scope.branches.includes(branch) && !grant.scope.branches.includes("*")) {
        throw new VibeKitError({
          ...
          message:
            `Branch ${branch} is outside the effective grant for ${capability}: ` +
            `choose an allowed branch or update the repository scope setting.`,
  ...
  if (grant.authorization === "deny") {
    throw new VibeKitError({
      ...
      message:
        `Authorization denies ${humanAction(input.invocation.action, capability)} on ${input.invocation.target}. ` +
        `Change the ${authorizationSetting(capability)} only when you intend to allow this action.`,
      details: {
        action: input.invocation.action,
        target: input.invocation.target,
        capability: input.invocation.capability,
  ...
  if (match === undefined) {
    throw new VibeKitError({
      ...
      message:
        `Explicit Approval is required for ${humanAction(input.invocation.action, capability)} on ${input.invocation.target}. ` +
        `Consequence: ${actionConsequence(capability)}. Approve this exact action in the pending Approval request.`,
      details: {
        action: input.invocation.action,
        target: input.invocation.target,
        capability: input.invocation.capability,
...
function capabilityLabel(capability: string): string {
  const labels: Readonly<Record<string, string>> = {
    "source.read": "file reading",
    "source.write": "file changes",
    "command.execute": "approved command execution",
    "web.fetch": "web page access",
    "web.search": "web search",
    "memory.read": "saved memory reading",
    "memory.write": "saved memory writing",
    "agent.delegate": "bounded delegation",
    "deploy.apply": "deployment",
    "destructive.delete": "permanent deletion",
    "project.configure": "Project configuration changes",
    "repository.write": "repository changes",
    "repository.issue.write": "sending a repository issue",
    "schedule.write": "scheduled task changes",
    "outbound.send": "sending an outbound message",
    "purchase.execute": "making a purchase",
  };
  if (labels[capability] !== undefined) {
    return labels[capability];
  }
  if (capability.startsWith("repository.")) return "repository access";
  if (capability.startsWith("schedule.")) return "scheduled work";
  return capability;
}

function capabilitySetting(capability: string): string {
  if (capability === "web.search") return "the Web search ability in setup";
  if (capability.startsWith("memory.")) return "the Memory ability in setup";
  if (capability === "source.read" || capability === "source.write") {
    return "the file access ability and workspace setting";
  }
  if (capability === "command.execute") return "the approved commands ability and workspace setting";
  return `the ${capabilityLabel(capability)} setting`;
}

function authorizationSetting(capability: string): string {
  if (capability === "destructive.delete") return "Project authorization for permanent deletion";
  if (capability === "deploy.apply") return "Project authorization for deployment";
  if (capability === "project.configure") return "Project authorization for Project configuration changes";
  return `Project authorization for ${capabilityLabel(capability)}`;
}

function humanAction(action: string, capability: string): string {
  const actionCapability = action.split(" / ").at(-1) ?? action;
  return actionCapability === capability ? capabilityLabel(capability) : action;
}

function actionConsequence(capability: string): string {
  if (capability === "source.write") return "files in the selected scope will be changed";
  if (capability === "command.execute") return "the requested command will run in the selected workspace";
  if (capability === "deploy.apply") return "the selected deployment target will be changed";
  if (capability === "destructive.delete") return "the selected data may be permanently removed";
  if (capability === "project.configure") return "Project behavior and future runs may change";
  return `the requested ${capabilityLabel(capability)} action will run`;
}
