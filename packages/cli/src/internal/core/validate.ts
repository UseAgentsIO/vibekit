...
import { normalizeProjectDocument } from "./project.js";
import type {
  DocumentKind,
  DocumentTypeMap,
  ValidationError,
...
export function parseAndValidateYaml<K extends DocumentKind>(
  ...
  if (kind === "project" && result.valid && result.data !== undefined) {
    return {
      ...result,
      data: normalizeProjectDocument(result.data as DocumentTypeMap["project"]) as DocumentTypeMap[K],
    };
  }
  return result;
}
