...
export async function isHostIpcAvailable(projectRoot: string): Promise<boolean> {
  return (await healthViaIpc(projectRoot)) !== undefined;
}

export async function healthViaIpc(projectRoot: string): Promise<HostHealth | undefined> {
  ...
  try {
    ...
    return response.ok === true ? response.result as HostHealth : undefined;
  } catch {
    return undefined;
  }
}
...
export async function shutdownViaIpc(
  projectRoot: string,
...
async function handleLine(
  ...
  try {
    ...
    if (request.type === "shutdown") {
      writeResponse(socket, { id: request.id, ok: true, result: { stopped: true } });
