...
export function approvePairing(projectRoot: string, code: string, now: Date = new Date()): PairedSender {
  ...
  writePairingStore(projectRoot, {
    schemaVersion: 1,
    owner: store.owner ?? Object.values(store.paired)[0] ?? record,
...
export function listPairings(projectRoot: string, now: Date = new Date()): PairingList {
  const store = readPairingStore(projectRoot);
  const pending = Object.values(store.pending).filter((entry) => Date.parse(entry.expiresAt) > now.getTime());
  return {
    ...(store.owner !== undefined
      ? { owner: store.owner }
      : Object.values(store.paired)[0] !== undefined
        ? { owner: Object.values(store.paired)[0] }
        : {}),
...
export function revokePairing(projectRoot: string, userId: string): boolean {
  ...
  const nextOwner = store.owner?.userId === userId ? Object.values(paired)[0] : store.owner;
  writePairingStore(projectRoot, {
    schemaVersion: 1,
...
function normalizeStore(parsed: Partial<PairingDocument>): PairingDocument {
  const owner = parsed.owner !== null && typeof parsed.owner === "object" && typeof parsed.owner?.userId === "string"
    ? parsed.owner
    : undefined;
  ...
  return { schemaVersion: 1, ...(owner !== undefined ? { owner } : {}), paired, pending };
}
