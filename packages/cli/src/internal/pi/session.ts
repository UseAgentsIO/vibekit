...
export async function createPiAgentSession(
  ...
  const tools = mergeToolNames(options.tools, options.customTools ?? []);
  const customTools = (options.customTools ?? []).map(adaptCustomTool);
...
export function adaptCustomTool(tool: PiCustomTool): Record<string, unknown> {
  return {
    ...tool,
    label: tool.name,
    execute: async (_toolCallId: string, params: unknown) => ({
      content: [{ type: "text", text: formatToolResult(await tool.execute(params)) }],
    }),
  };
}
