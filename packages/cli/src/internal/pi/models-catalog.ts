...
/**
 * Send one bounded, no-tool request through Pi's provider runtime. The caller
 * supplies a deployment credential in memory when one exists; this function
 * never persists it and returns only a redacted classification.
 */
export async function probeProvider(options: {
  ...
  const base = { provider: options.provider, model: options.model };
  if (options.provider.trim().length === 0 || options.model.trim().length === 0) {
    return { ...base, status: "model_unavailable", message: "A provider and model are required" };
  }

  let runtime: ModelRuntime;
  try {
    runtime = await ModelRuntime.create({ allowModelNetwork: false, refreshOnCreate: false });
  } catch (error) {
    return { ...base, status: "network_failure", message: redactProbeMessage(error) };
  }
  const resolved = runtime.getModel(options.provider, options.model);
  if (resolved === undefined) {
    return { ...base, status: "model_unavailable", message: "Pi does not have this model" };
  }

  const env = options.env ?? {};
  const configured = options.apiKey !== undefined && options.apiKey.length > 0
    || runtime.hasConfiguredAuth(options.provider)
    || (typeof env[secretNameForProvider(options.provider)] === "string"
      && env[secretNameForProvider(options.provider)]!.length > 0);
  if (!configured) {
    return { ...base, status: "missing_credentials", message: "Provider credentials are not configured" };
  }
  ...
  try {
    ...
    if (response.stopReason === "error") {
      return {
        ...base,
        status: classifyProbeMessage(response.errorMessage),
        message: redactProbeMessage(response.errorMessage),
      };
    }
    return { ...base, status: "ok" };
  } catch (error) {
    const message = redactProbeMessage(error);
    return { ...base, status: classifyProbeMessage(message), message };
  }
}

function classifyProbeMessage(message: string | undefined): Exclude<ProviderProbeStatus, "ok"> {
  const text = (message ?? "").toLowerCase();
  if (
    /model|unknown model|does not exist|not found/.test(text) &&
    !/network|connect|timeout|fetch/.test(text)
  ) {
    return "model_unavailable";
  }
  if (/401|403|unauthori[sz]ed|forbidden|invalid (?:api )?key|authentication|credential|quota|rate limit/.test(text)) {
    return "provider_rejection";
  }
  if (/enotfound|eai_again|econnrefused|etimedout|timeout|network|fetch failed|socket|connect/.test(text)) {
    return "network_failure";
  }
  return "provider_rejection";
}

function redactProbeMessage(error: unknown): string {
  const value = error instanceof Error ? error.message : String(error ?? "Provider probe failed");
  return value
